...
def upsert_companies(conn, companies: list[dict]) -> tuple[int, int]:
    ...
    with conn.cursor() as cur:
        # Keep the fixed-width CNPJ type on both sides so Postgres can use the
        # primary-key index instead of scanning the full companies table.
        cur.execute(
            "select btrim(cnpj::text) from mei_email.empresas where cnpj = any(%s::char(14)[])",
            (cnpjs,),
        )
